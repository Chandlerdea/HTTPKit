...
extension HTTPModelController {
    ...
    public func getModels<Model: Codable & Equatable>(with requestBuilder: HTTP.RequestBuilder, in session: URLSession, completion: @escaping (Result<[Model], Error>) -> Void) {
        let request: URLRequest = requestBuilder.build()
        self.sendRequest(request, in: session, completion)
    }

    public func getModel<Model: Codable & Equatable>(with requestBuilder: HTTP.RequestBuilder, in session: URLSession, completion: @escaping (Result<Model, Error>) -> Void) {
        let request: URLRequest = requestBuilder.build()
        self.sendRequest(request, in: session, completion)
    }

    public func postModel<Model: Codable & Equatable>(_ model: Model, with requestBuilder: HTTP.RequestBuilder, in session: URLSession, completion: @escaping (Result<Model, Error>) -> Void) {
        let request: URLRequest = requestBuilder.build()
        self.sendRequest(request, in: session, completion)
    }

    public func putModel<Model: Codable & Equatable>(_ model: Model, with requestBuilder: HTTP.RequestBuilder, in session: URLSession, completion: @escaping (Result<Model, Error>) -> Void) {
        let request: URLRequest = requestBuilder.build()
        self.sendRequest(request, in: session, completion)
    }

    public func deleteModel<Model: Codable & Equatable>(_ model: Model, with requestBuilder: HTTP.RequestBuilder, in session: URLSession, completion: @escaping (Result<Void, Error>) -> Void) {
        let request: URLRequest = requestBuilder.build()
